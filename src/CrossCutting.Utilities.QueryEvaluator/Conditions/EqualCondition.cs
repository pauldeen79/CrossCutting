...
public partial record EqualCondition
{
    ...
    public override Task<Result<bool>> EvaluateTypedAsync(ExpressionEvaluatorContext context, CancellationToken token)
        => ConditionHelper.EvaluateObjectConditionAsync(SourceExpression, CompareExpression, context, (first, second) => Equal.Evaluate(first, second, context.Settings.StringComparison), token);
}
