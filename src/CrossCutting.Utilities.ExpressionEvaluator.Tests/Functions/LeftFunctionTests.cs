...
public class LeftFunctionTests : TestBase<LeftFunction>
{
    public class Evaluate : LeftFunctionTests
    {
        ...
        {
            // Arrange
            var context = new FunctionCallContext(new FunctionCallBuilder().WithName("Left").AddArguments("\"hello world\"", "5"), CreateContext("Dummy"), MemberType.Function);
            var sut = CreateSut();
        ...
        {
            // Arrange
            var context = new FunctionCallContext(new FunctionCallBuilder().WithName("Left").AddArguments("\"hello world\"", "100000"), CreateContext("Dummy"), MemberType.Function);
            var sut = CreateSut();
