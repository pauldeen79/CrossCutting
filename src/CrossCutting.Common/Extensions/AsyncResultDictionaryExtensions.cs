...
{
    public static async Task<Result<T>> TryCastValueAsync<T>(this IReadOnlyDictionary<string, Func<Task<Result<object?>>>> instance, string resultKey)
        => (await instance.GetValueAsync(resultKey).ConfigureAwait(false))
            .TryCastAllowNull<T>();

    public static async Task<Result<object?>> GetValueAsync(this IReadOnlyDictionary<string, Func<Task<Result<object?>>>> instance, string resultKey)
