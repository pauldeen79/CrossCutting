...
public class Recursion : IMathematicExpression
{
    ...
    public Result<MathematicExpressionState> Parse(MathematicExpressionState state)
    {
        ...
        do
        {
            ...
            var subResult = state.Context.Parse(found);
            if (!subResult.Status.IsSuccessful())
            {
                return subResult.Status.ToTypedResult(state, subResult.ErrorMessage, subResult.ValidationErrors);
            }

            state.Remainder = state.Remainder.Substring(0, openIndex)
                + FormattableString.Invariant($"{MathematicExpression.TemporaryDelimiter}{state.ParseResults.Count}{MathematicExpression.TemporaryDelimiter}")
                + state.Remainder.Substring(closeIndex + 1);

            state.ParseResults.Add(subResult);
