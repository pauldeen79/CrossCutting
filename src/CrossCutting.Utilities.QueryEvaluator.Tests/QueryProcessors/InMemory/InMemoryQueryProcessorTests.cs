...
public sealed class InMemoryQueryProcessorTests : TestBase
{
    ...
    public async Task Can_Find_One_Item()
    {
        ...
        var query = new SingleEntityQueryBuilder()
            .AddConditions(new EqualConditionBuilder()
                .WithSourceExpression(new PropertyNameExpressionBuilder().WithPropertyName(nameof(MyEntity.Property1)))
                .WithCompareExpression(new LiteralExpressionBuilder("A")))
            .AddSortOrders(new SortOrderBuilder(new PropertyNameExpressionBuilder().WithPropertyName(nameof(MyEntity.Property2)), SortOrderDirection.Ascending))
            .Build();
    ...
    public async Task Can_Use_Nested_Property_In_Query()
    {
        ...
        var query = new SingleEntityQueryBuilder()
            .AddConditions(new EqualConditionBuilder()
                .WithSourceExpression(new PropertyNameExpressionBuilder().WithSourceExpression(new PropertyNameExpressionBuilder().WithPropertyName(nameof(MyNestedEntity.Property))).WithPropertyName(nameof(MyEntity.Property1)))
                .WithCompareExpression(new LiteralExpressionBuilder("A")))
            .Build();
    ...
    {
        // Arrange
        var parser = new SingleEntityQueryParser<SingleEntityQueryBuilder, PropertyNameExpressionBuilder>(() => new PropertyNameExpressionBuilder().WithPropertyName("MyProperty"));
